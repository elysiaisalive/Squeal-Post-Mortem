function __input_system_tick()
{
    ...
    //Block mouse capture when window cState changes
            ...
            if (_global.__frame - _global.__mouse_capture_frame > 10)
            {
                ...
                ||  (abs(_pointer_y - window_get_height()/2) >= 1))
                {
                    ...
                    repeat(INPUT_COORD_SPACE.__SIZE)
                    {
                        ...
                        //Only detect movement in the display coordinate space so that moving a room's CURRENT_VIEW, or moving the window, doesn't trigger movement
            ...
            if (is_struct(_gamepad))
            {
                if (gamepad_is_connected(_g))
                {
                    if ((os_type == os_switch) && (_gamepad.description != gamepad_get_description(_g)))
                    {
                        ...
                        //Instead we have to check for changes in the description to see if cState has changed
                        _gamepad.discover();
                    }
                    else
                    {
