...
if (solid = 1)
	exit
/*other.object_index=objDogPatrol or*/
if (other.object_index = objEnemyFat) or (other.object_index = objInspector)  {
	swinger = 4
	if (abs(swingspeed) < 2)
		scr_playsound(sndDoorOpen)
...
if abs(swingspeed)>3.5 {
	if swinger=1 or swinger=0 {
		...
		if global.maskindex=4 {
			...
			my_id = instance_create_depth(other.x,other.y,0,objKnockedOut)
		...
		if noweapon=0 {
			...
			with objEnemy
				if alert=1
					global.test+=1
			my_id=instance_create_depth(x,y-12,0,objScore)
			...
			global.kills+=1
			my_id=instance_create_depth(other.x,other.y,0,objWeaponThrow)
			if swingspeed>0
				my_id.direction=90-15+random(30)+25
			else
				my_id.direction=255+random(30)+25
...
swinger=2
if abs(swingspeed)<2 scr_playsound(sndDoorOpen)
